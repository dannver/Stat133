...
y15 %>% #entire year, without outliers
  filter(distance < 50, total_fare > 0) %>%
  ggplot(aes(y = total_fare, x = distance)) +
  geom_point() +
...
y15_month <- y15 %>%
  mutate(month = lubridate::month(pickup_time))

y15_month_ave_fare <- y15_month %>%
  filter(total_fare > 0, total_fare <100)
  group_by(month)%>%
  summarize(ave=mean(total_fare))
...
y15_day <- y15 %>%
  mutate(weekday = lubridate::wday(pickup_time))
...
```{r}
y15_pickup <- y15 %>%
  mutate(coordinates=paste(y15$pickup_long, y15$pickup_lat, 0, sep=", "))
...
```{r}
y15_dropoff <- y15 %>%
  mutate(coordinates=paste(y15$dropoff_long, y15$dropoff_lat, 0, sep=", "))
...
```{r}
y15_passengers <- y15 %>%
  group_by(number_of_passengers) %>%
  summarize(tot = n())
  
y15_passengers %>%
  ggplot(aes(x = number_of_passengers, y = tot))
  geom_bar(stat ="identity")
```